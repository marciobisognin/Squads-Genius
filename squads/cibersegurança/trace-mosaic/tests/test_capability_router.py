...
class CapabilityRouterTests(unittest.TestCase):
 ...
 def test_route_is_non_executing(self):
  p=self.run_cli('route','--technique','osint-source-planning','--context','public','--band','0');self.assertEqual(p.returncode,0);d=json.loads(p.stdout);self.assertFalse(d['execution_performed']);self.assertIn(d['decision'],['PLAN_ONLY','GATED_HANDOFF'])
