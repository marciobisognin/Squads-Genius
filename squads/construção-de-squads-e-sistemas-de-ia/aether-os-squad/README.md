...
# 🌌 AETHER OS — Sistema Operacional Cognitivo de Squads e Organizações de Agentes
...
Agora com **organizações que vivem entre runs** e **personas que vestem método, não juízo**.*

<br>

![version](https://img.shields.io/badge/versão-1.1.0_(PRD_v1.3)-1FA87E?style=for-the-badge)
...
![agents](https://img.shields.io/badge/🧠_mentes-13_+_Cortex-1a202c?style=flat-square)
![engines](https://img.shields.io/badge/⚙️_motores_determinísticos-12-1a202c?style=flat-square)
![tasks](https://img.shields.io/badge/📋_tasks-16-1a202c?style=flat-square)
![workflows](https://img.shields.io/badge/🔀_workflows-7-1a202c?style=flat-square)
![gates](https://img.shields.io/badge/🛡️_gates-Cynefin_+_HITL_+_ELENCHUS-c53030?style=flat-square)
![oikos](https://img.shields.io/badge/🏛️_oikos-organizações_persistentes-6d28d9?style=flat-square)
![prosopon](https://img.shields.io/badge/🎭_prósopon-personas_com_proveniência-6d28d9?style=flat-square)
...
| 🔨 [Forja de squads](#-forja-quando-não-existe-squad-adequado) | 🧬 [Autoaprendizado](#-autoaprendizado-governado) | 🏛️ [Oikos (v1.3)](#️-oikos-organizações-que-vivem-entre-runs) |
| 🎭 [Prósopon (v1.3)](#-prósopon-persona-é-máscara-não-juízo) | 🔌 [Multi-host & economia (v1.3)](#-host-é-driver-economia-é-neutra) | 🚀 [Início rápido](#-início-rápido) |
...
| 🏛️ **OIKONÓMOS** | οἰκονόμος, o administrador | Gere oikoi: pulso, inbox, cargos, memória | `oikos_engine.py` (ticks, rotas, teto) |
| 🎭 **RETRATISTA** | retrato do **método**, não da pessoa | Extrai prósopa de obra pública com proveniência | `persona_engine.py` + revisão humana |
...
| Replay | `replay_engine.py` | reexecução + comparação **byte a byte** (inclui derivação e rotas de oikos) | `aether.replay-report/v1` |
| Oikos *(v1.3)* | `oikos_engine.py` | manifesto, ticks do pulso (cron + relógio injetável), rotas, teto de autonomia, ciclo de vida | `aether.pulse-report/v1` · `aether.inbox-routing/v1` |
| Personas *(v1.3)* | `persona_engine.py` | contrato do prósopon, rótulo na egressão, antipersonificação, Galeria | `aether.prosopon-validation/v1` · `aether.persona-egress-check/v1` |
| Host *(v1.3)* | `host_adapter.py` | capacidades declaradas + degradação determinística + paridade | `aether.host-capabilities/v1` |
| Economia *(v1.3)* | `token_economy.py` | 4 regras de derivação, teto por envelope, layout cacheável | `aether.derivation-decision/v1` |
...
## 🏛️ Oikos: organizações que vivem entre runs

> [!IMPORTANT]
> **Organização não executa; agenda.** Um oikos (οἶκος, a unidade produtiva da
> casa grega) persiste cargos, processos, agenda e memória — mas todo trabalho
> acontece em **runs normais**, com os mesmos motores, gates, orçamento e trilha.

- **Organograma e cargos** com mandato, capacidades exigidas e **teto de
  autonomia**: risco acima do teto escala pela cadeia até o gate humano.
- **Pulso**: agenda cron declarativa; cada disparo abre um **run de ciclo** com
  orçamento debitado do oikos. Ciclo perdido gera `PulseTick` + alerta — nunca
  acúmulo silencioso.
- **Inbox**: briefings passam pela classificação de intake normal (KRITÉS) e
  são roteados ao cargo pelo organograma; sem rota, escala.
- **Memória em 3 camadas** (oikos / cargo com TTL / run) com proveniência e
  exclusão em cascata; **modo não assistido** só nos riscos declarados —
  não assistido nunca significa sem gates.
- Ciclo de vida `draft → active → paused → archived`; limites de oikoi e
  ciclos simultâneos impostos pelo Motor de Quotas.

```bash
python3 scripts/oikos_engine.py validate --manifest examples/oikos_auditoria.yaml
python3 scripts/oikos_engine.py pulse-due --manifest examples/oikos_auditoria.yaml --now 2026-07-06T07:00:00
```

<br>

## 🎭 Prósopon: persona é máscara, não juízo

> [!CAUTION]
> Um prósopon (πρόσωπον, a máscara do teatro grego) modula **como** uma proposta
> é formulada e comunicada — **nunca o que é decidido**. Nenhum campo de persona
> é lido por motor determinístico.

- Perfil versionado em **5 camadas** — princípios, modelos mentais,
  heurísticas, métodos, voz — com **proveniência item a item** de obra pública.
- Produzido pela constelação **Retratista**: extração → consolidação
  (SÝNTHESIS) → contraditório (ELENCHUS: item sem lastro é eliminado) →
  fidelidade → revisão humana → **Galeria**.
- **Salvaguardas verificadas em código** (`persona_engine.py`): rótulo
  "conteúdo sintético inspirado em método publicado" obrigatório na egressão;
  **antipersonificação** (assinar/apresentar-se como a pessoa ⇒
  `policy_denied` + evento de segurança); **zero atribuição factual**;
  despublicação auditada a pedido do titular.

```bash
python3 scripts/persona_engine.py validate --prosopon examples/prosopon_exemplo.json
python3 scripts/persona_engine.py label-check --artifact <artefato> --persona <id>
```

<br>

## 🔌 Host é driver, economia é neutra

**Multi-host (§9.4/9.5):** o Hermes é o host de referência; qualquer host entra
por contrato `aether.host-adapter/v1` declarando capacidades (`subagent_fork`,
`prompt_caching`, `native_sandbox`…). Capacidade ausente segue a **tabela de
degradação determinística** — sem `subagent_fork`, derivação vira instanciação
limpa; sem `approval_ui`, o gate aponta para a CLI. Paridade entre hosts é
**verificada por suíte golden, não presumida**.

**Economia de tokens (§26.5):** cinco alavancas determinísticas — carga
preguiçosa, carga dirigida, layout cacheável, derivação com herança, handoff
por referência — com regras de derivação em ordem fixa e **segurança
precedendo economia** (conteúdo não confiável ⇒ instanciação limpa, sempre).
**Nenhuma alavanca pode mudar uma decisão**: o decision-replay compara pares
com/sem cache e com/sem herança; divergência é incidente crítico.

```bash
python3 scripts/host_adapter.py capabilities --adapter examples/host_hermes.json
...
| `aether.oikos/v1` *(v1.3)* | manifesto do oikos | organograma, cargos, pulso, inbox, políticas |
| `aether.pulse-tick/v1` *(v1.3)* | oikos_engine | disparo do pulso + status (nunca silêncio) |
| `aether.prosopon/v1` *(v1.3)* | Retratista | 5 camadas com proveniência + rótulos + ciclo de vida |
| `aether.host-adapter/v1` *(v1.3)* | contrato do host | capacidades declaradas + degradação |
| `aether.derivation-decision/v1` *(v1.3)* | token_economy | herança vs. limpa, regra aplicada, evento |
...
**Aceite verificado nesta versão (1.1.0):** `doctor` saudável (23/23 checks); `demo` com
...
loop de revisão convergindo em 2 tentativas; **oikos** validado com pulso correto
(segunda 07h devido, domingo não), roteamento com escalada e teto de autonomia
bloqueando risco alto; **persona** publicada na Galeria, egressão sem rótulo/com
assinatura **bloqueada** (evento de segurança) e despublicação auditada; **derivação**
aplicando as 4 regras na ordem com neutralidade `pass` no replay.
...
|---|---|
| Runtime anfitrião | **Hermes Agent** (referência) + qualquer host via `aether.host-adapter/v1` com degradação determinística |
