...
<p>
  <img src="https://img.shields.io/badge/status-premium-8A2BE2?style=for-the-badge" />
  <img src="https://img.shields.io/badge/domain-super--squads-00B894?style=for-the-badge" />
  <img src="https://img.shields.io/badge/foco-produto%20%7C%20oferta%20%7C%20MVP-F59E0B?style=for-the-badge" />
...
```mermaid
flowchart TD
  A[Ideia, vídeo, pesquisa ou oportunidade] --> B[Guardião de Ianus-Kree]
  B --> C[Leitura de contexto e roteamento]
  C --> D[Pesquisa e benchmarking]
  D --> E[Desenho da oferta]
  E --> F[Arquitetura de squads e componentes]
  F --> G[Design, MVP e automações]
  G --> H[Quality gates, riscos e validação]
  H --> I[Roadmap, documentação e handoff]
  I --> J[Pacote final executável]
```
...
```mermaid
flowchart LR
  A[Intake estratégico] --> B[Pesquisa Nemeton]
  B --> C[Oferta Cerridwen]
  C --> D[Arquitetura Brigid]
  D --> E[Design Sersi]
  D --> F[MVP Nuada]
  E --> G[Integrações Manannán]
  F --> G
  G --> H[Validação Eirene]
  H --> I[Roadmap Ananke]
  I --> J[Documentação Ariadne]
  J --> K[Métricas Taranis]
  K --> L[Entrega final]
```
...
> 1. **Dê contexto** ao assistente apontando os arquivos do squad (especialmente `squads/anvil-of-annwn-super-squad-system/squad.yaml` e `squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml`).
> 2. **Peça que ele assuma a persona do orquestrador** (veja os agentes em `squads/anvil-of-annwn-super-squad-system/agents/`).
> 3. **Conduza o fluxo** respeitando os checkpoints humanos e validando cada handoff/contrato.
>
> **Prompt de ativação** (copie, cole e ajuste o briefing):
> ```text
> Assuma a persona do orquestrador do squad (veja os agentes em `squads/anvil-of-annwn-super-squad-system/agents/`)
> e conduza o fluxo definido em `squads/anvil-of-annwn-super-squad-system/`. Siga `squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml`.
...
> Leia @squads/anvil-of-annwn-super-squad-system/squad.yaml e assuma a persona do orquestrador do squad.
  Siga @squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml. Conduza o fluxo para o briefing: <...>
```
...
2. No **Chat / Composer (⌘/Ctrl + I)**, referencie os arquivos com `@`:
   ```text
   @squads/anvil-of-annwn-super-squad-system/squad.yaml @squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml
   Assuma a persona do orquestrador e conduza o fluxo para o briefing: <...>
   ```
3. **Persistente:** crie um `.cursorrules` na raiz apontando para `squads/anvil-of-annwn-super-squad-system/` como squad ativo.
...
```text
@workspace #file:squads/anvil-of-annwn-super-squad-system/squad.yaml #file:squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml
...
```text
Leia squads/anvil-of-annwn-super-squad-system/squad.yaml e assuma a persona do orquestrador.
Conduza o fluxo do squad e execute os scripts em squads/anvil-of-annwn-super-squad-system/scripts/ quando o passo pedir.
Briefing: <...>
```
...
- **Continue.dev:** use `@file` para `squads/anvil-of-annwn-super-squad-system/squad.yaml`; cole o prompt de ativação.
- **Aider:** `aider squads/anvil-of-annwn-super-squad-system/squad.yaml` e instrua o orquestrador.
- **ChatGPT / Gemini (sem acesso a arquivos):** copie o conteúdo de `squads/anvil-of-annwn-super-squad-system/squad.yaml` e `squads/anvil-of-annwn-super-squad-system/workflows/annwn-forge-main.yaml` para o chat, cole o prompt de ativação e rode eventuais scripts localmente, colando a saída de volta.
