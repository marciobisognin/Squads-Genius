...
<p>
  ...
  <img src="https://img.shields.io/badge/domain-incubadoras%20%7C%20CERNE%20%7C%20inova%C3%A7%C3%A3o-0984E3?style=for-the-badge" alt="Domínio" />
  <img src="https://img.shields.io/badge/license-MIT-6C5CE7?style=for-the-badge" alt="Licença MIT" />
</p>
...
```mermaid
flowchart TD
    A[📚 Documentos de entrada<br/>regimentos, resoluções, PDFs, manuais e editais] --> B[🔎 Leitura técnica e diagnóstico]
    B --> C[🏛️ Arquitetura normativa]
    B --> D[🧬 Metodologia CERNE]
    C --> E[⚙️ Sistema operacional da incubadora]
    D --> E
    E --> F[📄 Fábrica documental]
    E --> G[📊 Painel de indicadores]
    F --> H[🎨 Publicação A4 Premium]
    G --> H
    H --> I[🛡️ Validação final]
    I --> J[📦 Entrega completa<br/>documentos, relatório, HTML, PDF e pacote validado]
```
...
```mermaid
flowchart LR
    N[🏛️ Arquiteto<br/>Normativo] --> C[🧬 Metodologista<br/>CERNE]
    C --> O[⚙️ Designer de<br/>Sistemas]
    O --> D[📄 Fábrica<br/>Documental]
    O --> I[📊 Cartógrafo de<br/>Indicadores]
    D --> P[🎨 Publicador<br/>A4 Premium]
    I --> P
    P --> Q[🛡️ Guardião de<br/>Qualidade]
```
...
```mermaid
flowchart TB
    A[Entrada: documentos institucionais] --> B[Interpretação técnica]
    B --> C[Organização normativa]
    C --> D[Modelo de operação]
    D --> E[Documentos e indicadores]
    E --> F[Relatório visual]
    F --> G[PDF + HTML + ZIP]
    G --> H[Entrega institucional pronta]
```
...
> 1. **Dê contexto** ao assistente apontando os arquivos do squad (especialmente `IFFar-Squads/squads/vetorviveiro-squad/squad.yaml` e `IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml`).
> 2. **Peça que ele assuma a persona do orquestrador** (veja os agentes em `IFFar-Squads/squads/vetorviveiro-squad/agents/`).
> 3. **Conduza o fluxo** respeitando os checkpoints humanos e validando cada handoff/contrato.
>
> **Prompt de ativação** (copie, cole e ajuste o briefing):
> ```text
> Assuma a persona do orquestrador do squad (veja os agentes em `IFFar-Squads/squads/vetorviveiro-squad/agents/`)
> e conduza o fluxo definido em `IFFar-Squads/squads/vetorviveiro-squad/`. Siga `IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml`.
...
> Leia @IFFar-Squads/squads/vetorviveiro-squad/squad.yaml e assuma a persona do orquestrador do squad.
  Siga @IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml. Conduza o fluxo para o briefing: <...>
```
...
2. No **Chat / Composer (⌘/Ctrl + I)**, referencie os arquivos com `@`:
   ```text
   @IFFar-Squads/squads/vetorviveiro-squad/squad.yaml @IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml
   Assuma a persona do orquestrador e conduza o fluxo para o briefing: <...>
   ```
3. **Persistente:** crie um `.cursorrules` na raiz apontando para `IFFar-Squads/squads/vetorviveiro-squad/` como squad ativo.
...
```text
@workspace #file:IFFar-Squads/squads/vetorviveiro-squad/squad.yaml #file:IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml
...
```text
Leia IFFar-Squads/squads/vetorviveiro-squad/squad.yaml e assuma a persona do orquestrador.
Conduza o fluxo do squad e execute os scripts em IFFar-Squads/squads/vetorviveiro-squad/scripts/ quando o passo pedir.
Briefing: <...>
```
...
- **Continue.dev:** use `@file` para `IFFar-Squads/squads/vetorviveiro-squad/squad.yaml`; cole o prompt de ativação.
- **Aider:** `aider IFFar-Squads/squads/vetorviveiro-squad/squad.yaml` e instrua o orquestrador.
- **ChatGPT / Gemini (sem acesso a arquivos):** copie o conteúdo de `IFFar-Squads/squads/vetorviveiro-squad/squad.yaml` e `IFFar-Squads/squads/vetorviveiro-squad/workflows/pipeline-operacional.yaml` para o chat, cole o prompt de ativação e rode eventuais scripts localmente, colando a saída de volta.
